...
public class Cube {
	...
	public Cube() {
		...
		int colors[] =
			{0, 0, 0, one, one, 0, 0, one, one, one, 0, one, 0, one, 0, one, 0, 0, one, one, one,
				0, one, one, one, one, one, one, 0, one, one, one,};
		byte indices[] =
		...
		mColorBuffer = ByteBuffer.allocateDirect(colors.length * 4).order(ByteOrder.nativeOrder()).asIntBuffer();
